...
## ☕ 프로젝트 결론
### 커피 물가 상승과 음식점/카페 개폐업 간의 관계
2022년에는 커피 물가를 포함한 외식 업계 전반에서 물가 상승 이슈가 있었다. 커피 물가 상승은 일시적으로 카페의 폐업에 영향을 주었으나, 점진적으로 하락세인 것이 관찰된다. 외식비 상승에 의해 영향을 크게 받은 일반 음식점의 개업 및 폐업과 달리, 커피 가격 상승으로 인한 카페 분포의 변화는 보다 적을 것으로 보인다. 커피 가격이 상승하였음에도 원두 수입량 및 카페의 개업 수치는 증가하고 있음으로 보아, 서울시 내 커피 소비량은 계속해서 증가할 것으로 보여진다.
### 행정구별 종사자 수와 카페 시장 간의 관계
물가 외 카페 시장에 영향을 미치는 요인을 찾아보기 위해 서울 행정구별 종사자 수를 조사해보았다. 종사자수가 많다고 해서 카페 시장이 안정적으로 유지되지는 않았다. 지표가 안정적인 행정구도 있었지만 폐업 수가 더욱 많은 행정구들도 있었는데, 이는 코로나로 인한 재택 근무가 증가하면서 소비량이 줄어 카페 폐업 수가 늘어났을 것으로 추측된다.
### 아쉬운 점
커피 소비량 데이터 수집 부분에서 아쉬움이 있었다. 소비량 대신 사용한 원두 수입량은 커피 소비량과의 관계보다는 수확 계절에 영향을 많이 받았기에 프로젝트에서 원하던 것과는 어긋나있는 데이터였다. 데이터 수집에 있어서, 어떻게 데이터를 수집하고 확보할 것인지, 이 데이터가 분석하고자 하는 주제에 관련성이 얼마나 있는지 분석하는 능력도 중요함을 느꼈다.
